...
import { schemeCategory10, interpolatePRGn } from 'd3-scale-chromatic';
...
class Words extends Component {
  ...
  draw = (svg, data, layout) => {
    const color = (length) => scaleLinear().interpolate(interpolatePRGn).domain([0, length]).range(['yellow', 'blue']);
    select(svg).append("g")
      .attr('transform', `translate(${layout.size()[0] / 2},${layout.size()[1] / 2})`)
      .selectAll("text")
      .data(data)
      .enter().append('text')
      .style('fill', (d, i) => color(d.length))
