...
class Words extends Component {
  ...
  draw = (svg, data, layout) => {
    select(svg).append("g")
      ...
      select(svg).selectAll("text").on("mouseover",function(){ select(this).transition().attr("transform","scale(1.2)") });
      select(svg)..selectAll("text").on("mouseout",function(){ select(this).transition().attr("transform","scale(1)") });
  }
