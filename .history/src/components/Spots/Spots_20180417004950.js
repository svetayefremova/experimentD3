...
class Spots extends Component {
  ...
  createSpots = () => {
    ...
    for (let el of data) {
      ...
      const angle = Math.floor(Math.random() * (48 - 43) + 43);
    ...
    range(50).map(() => {
      ...
      if (!ctx.isPointInPath(heart, x, y)) {
        ...
        const color = colorScale(Math.random());
        const radius = WIDTH * (0.1 + Math.random() * 0.15);
        const angle = Math.floor(Math.random() * (48 - 43) + 30);
  ...
  drawCanvas(nodes, links, ctx) {
    ...
    // bubbles
    nodes.forEach((d) => {
      const segment = Math.floor(Math.random() * (48 - 43) + 30);
      ctx.beginPath();
      this.drawCurve(d, segment, ctx);
  ...
  drawCurve(d, segment, ctx) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      ...
      const cp1x = p1.x + (p2.x - p0.x) / 6 * 2;
      const cp1y = p1.y + (p2.y - p0.y) / 6 * 2;

      const cp2x = p2.x - (p3.x - p1.x) / 6 * 2;
      const cp2y = p2.y - (p3.y - p1.y) / 6 * 2;
