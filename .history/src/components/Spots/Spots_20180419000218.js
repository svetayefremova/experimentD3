    ...
    range(2).map(() => {
      function findBestPoints(nodes) {  
        ...
        if (!ctx.isPointInPath(heart, x, y)) {
          ...
          const isPointInRadius = nodes.filter(point => {
            const circle = [point.x, point.y, searchRadius];
            console.log(this.pointInCircle([x, y], circle));
            return this.pointInCircle([x, y], circle);
          });
          //console.log(isPointInRadius);
      ...
      findBestPoints.call(this, nodes);
