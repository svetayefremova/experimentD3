...
class Spots extends Component {
  ...
  createSpots = () => {
    ...
    range(70).map(() => {
      return this.findBestPoints(nodes, ctx, heart, colorScale);
    });
  ...
  findBestPoints(nodes, ctx, heart, colorScale) {
    ...
    if (!ctx.isPointInPath(heart, x, y)) {
      ...
      const isPointInRadius = nodes.find(point => {
        const circle = [point.x, point.y, searchRadius];
        return this.pointInCircle([x, y], circle) === true;
      });

      if (isPointInRadius) {
        // recursion if point is too close too current nodes
        this.findBestPoints(nodes, ctx, heart, colorScale);
      } else {
        const color = colorScale(Math.random());
        const radius = WIDTH * (0.1 + Math.random() * 0.15);
