...
class Spots extends Component {
  ...
  createSpots = () => {
    ...
    for (let el of data) {
      ...
      Object.assign(el, { color, radius: 0, max_radius: radius });
    }
    ...
    range(50).map(() => {
      ...
      if (!ctx.isPointInPath(heart, x, y)) {
        ...
        const color = colorScale(Math.random());
        const radius = WIDTH * (0.1 + Math.random() * 0.25);
        nodes.push({x, y, color, radius: 0, max_radius: radius});
      }
  ...
  drawCanvas(nodes, links, ctx) {
    ...
    nodes.forEach((d) => {
      ctx.beginPath();
      ctx.moveTo(d.x + d.radious, d.y);
      ctx.arc(d.x, d.y, d.radious, 0, 2 * Math.PI, false);
