...
class Spots extends Component {
  ...
  createSpots = () => {
    ...
    range(2).map(() => {
      ...
      let node = diagram.find(x, y, 100);
      console.log('find', node, { x, y } );

      if (!ctx.isPointInPath(heart, x, y) && node) {
        do {
          x = Math.random() * WIDTH;
          y = Math.random() * HEIGHT;
          node = diagram.find(x, y, 100);
        } while (!ctx.isPointInPath(heart, x, y) && node)
      } else {
        diagram = d3Voronoi(nodes);
    ...
    console.log(nodes);
    const d3Voronoi2 = voronoi()
     .x(d => d.x)
     .y(d => d.y);
    const diagramm = d3Voronoi2(nodes);
    const links = diagramm.links();
    // draw voronoi canvas
  ...
  drawCanvas(nodes, links, ctx) {
    ...
    nodes.forEach((d) => {
      ...
      ctx.font = "10px Arial";
      ctx.fillStyle = "red";
      ctx.fillText(`${d.x}, ${d.y}`, d.x + 3, d.y - 3)
    });
