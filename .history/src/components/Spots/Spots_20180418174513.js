...
import { quadtree, extent } from 'd3-quadtree';
...
class Spots extends Component {
  ...
  createSpots = () => {
    ...
    range(50).map(() => {
      ...
      const d3Quadtree = quadtree(nodes).extent([[0, 0], [WIDTH, HEIGHT]]);
      console.log(d3Quadtree);
      ...
      if (!ctx.isPointInPath(heart, x, y)) {
        ...
        this.search(d3Quadtree, rx1, rx2, ry1, ry2);
  ...
  search(quadtree, rx1, rx2, ry1, ry2) {
    console.log(quadtree.root());
    //console.log('d3Quadtree', rx1, rx2, ry1, ry2);
    quadtree.visit((node, x1, y1, x2, y2) => {
      console.log(node);
      if (!node.length) {
        do {
          console.log('d3Quadtree', node.data); 
        } while (node.next);
      }
    });  
  }
