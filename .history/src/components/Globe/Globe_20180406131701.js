...
class Globe extends Component {
  ...
  createGlobe = () => {
    ...
    const featureData = selectAll(".country").data(WorldData.features);
    const realFeatureSize = extent(featureData, d => geoArea(d));
    const newFeatureColor = scaleQuantize().domain(realFeatureSize).range(interpolatePuRd);
    selectAll(".country")
      .style("fill", d => newFeatureColor(geoArea(d)));
  };
