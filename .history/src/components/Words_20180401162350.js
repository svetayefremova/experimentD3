...
import { schemeCategory10, interpolateLab } from 'd3-scale-chromatic';
...
class Words extends Component {
  ...
  draw = (svg, data, layout) => {
    const color = (length) => scaleLinear().interpolate(interpolateLab).domain([0, length]).range(['yellow', 'blue']);
    select(svg).append("g")
