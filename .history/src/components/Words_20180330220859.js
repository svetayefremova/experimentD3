...
class Words extends Component {
  ...
  draw = (svg, data, layout) => {
    select(svg).append("g")
      ...
      select(svg).on("mouseover",function(){ select(svg).transition().attr("transform","scale(1.2)") });
      select(svg).on("mouseout",function(){ select(svg).transition().attr("transform","scale(1)") });
  }
