...
class Map extends Component {
  ...
  createMap = () => {
    ...
    selectAll("path.countries")
      .on("mouseover", (d) => this.centerBounds(view.node(), path.bounds(d), path.centroid(d), this.state.zoomK))
    ...
    this.mapZoom = zoom()
      ...
      .on("zoom", () => {
        ...
        this.setState({ zoomK: event.transform.k })
      });
  
    select('svg').call(this.mapZoom);
  };

  centerBounds(node, bounds, center, k) {
    select(node)
      .append("rect")
      .attr("class", "bbox")
      .attr("x", bounds[0][0])
    ...
    console.log(k);
    const radius = k * 5 >= 5 ? 5 : k * 5 <= 3 ? 3 : k * 5;
    console.log(radius);
    select(node)
      .append("circle")
      .attr("class", "centroid")
